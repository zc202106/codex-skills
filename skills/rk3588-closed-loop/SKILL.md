...
- 在 Windows 侧执行仓库脚本时，优先使用 `pwsh -NoProfile -ExecutionPolicy Bypass -File ...`，不要默认使用 `powershell -File ...`。
...
- 如果必须做一次性远端排查，优先使用仓库公共脚本封装；若不得不用直接远端命令，必须带连接超时与整体超时，并确保远端启动命令使用 `nohup ... </dev/null >/tmp/<name>-codex-start.log 2>&1 &` 这类真正脱离会话的形式。
...
```powershell
pwsh -NoProfile -ExecutionPolicy Bypass -File .\scripts\build-rk3588.ps1 -Program rk_ptz_vi
pwsh -NoProfile -ExecutionPolicy Bypass -File .\scripts\deploy-rk3588.ps1 -Program rk_ptz_vi
pwsh -NoProfile -ExecutionPolicy Bypass -File .\scripts\run-rk3588.ps1 -Program rk_ptz_vi
pwsh -NoProfile -ExecutionPolicy Bypass -File .\scripts\fetch-log-rk3588.ps1 -Program rk_ptz_vi -TailLines 120
pwsh -NoProfile -ExecutionPolicy Bypass -File .\scripts\pull-full-log-rk3588.ps1 -Program rk_ptz_vi
```
